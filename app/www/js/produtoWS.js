...
function autoCompleteMarca(){
    ...
    $.ajax({
        type: 'POST'
        , url: "http://10.18.3.35/Servidor/Produto.asmx/autocompleteMarca"
        , crossDomain:true
...
function autoComplete(){
    ...
    $.ajax({
        type: 'POST'
        , url: "http://10.18.3.35/Servidor/Produto.asmx/autocomplete"
        , crossDomain:true
        , contentType: 'application/json; charset=utf-8'
        , dataType: 'json'
...
function pesquisarProduto()
{
    ...
    if(nome!="" && (marca!="" || marca=="" ) && embalagem==""){
        ...
        url   = "http://10.18.3.35/Servidor/Produto.asmx/pesquisarProdutosNome";
    }
    
    //------ Pesquisar por marca -----//
    if(nome == "" && marca != "" && embalagem == ""){
        dados = "{idUsuario:'"+ID_USUARIO+"',token:'"+TOKEN+"',marca:'"+marca+"'}"
        url = "http://10.18.3.35/Servidor/Produto.asmx/pesquisarProdutosMarca" 
    }

    //------ Pesquisar por embalagem ----//
    if(nome != "" && marca != "" && embalagem != ""){
        dados =  "{idUsuario:'"+ID_USUARIO+"',token:'"+TOKEN+"',marca:'"+marca+"',nome:'"+nome+"',embalagem:'"+embalagem+"'}"
        url = "http://10.18.3.35/Servidor/Produto.asmx/pesquisarProdutosEmbalagem"
    }
